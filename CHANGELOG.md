...
- New wallet CLI option `taco wallet sign_message`
...
- Tacovdf updated to 1.0.7
- Tacopos updated to 1.0.11
- Clvm_tools updated to 0.4.5
- Taco_rs updated to 0.1.14
...
- Removed "Total iterations since start" from `taco show -s`
- Removed rate-limited wallet
- Removed the beta program link from the warning in the CLI
- Removed `--enable-data-server` from `taco configure` CLI
...
- Dropped unused `taco_minor_release_number`
- Just `raise`, not `raise e` when reraising
- Optimized `simple_solution_generator()`
- Allow developers to easily use standard Taco `clvm` puzzles and libraries
- Skipped validating `GTElement` in mempool
- Improved logging for `taco plotters version` errors
...
- Minter DID now shown in output of `taco wallet nft get_info` and in GUI
...
- Deprecated `taco plotters install` command
...
- Fixed taco farm summary if using a remote full node (thanks @yan74)
...
- Fixed the run_block utility to use tacolisp_deserialization
...
- Fixed `taco show -s` with other options as well
...
- Show Usage when running `taco plotters` (#13690)
...
- Fixed issues where TacoLisp was compiled during import requiring write access to the directory (#11257) (thanks @lourkeur). To force compilation, developers can set environment variable `TACO_DEV_COMPILE_CLVM_ON_IMPORT`
...
## 1.6.0 Taco blockchain 2022-9-20
...
- Fixed GUI issue for CAT offers where the CAT Tail would not show in the tooltip for `Unknown CAT`s (https://github.com/Taco-Network/taco-blockchain-gui/issues/950)

### Known Issues

- The CLI command `taco configure --enable-data-server`, and the `config.yaml` parameter at `data_layer.run_server` have no effect, and will be removed in the future
...
## 1.5.1 Taco blockchain 2022-8-23

### Added

- Add Source and Changelog to project_urls (Thanks @strayer!)
- Add condition code constant for REMARK, an always true Tacolisp condition
- Add several wallet optimizations
- Add `taco db backup --backup-file <backup_file_destination>` (Thanks @neurosis69!)
...
- Documented deserialization length limitations (8191 bytes) in CLVM ROM. We recommend using a local version of the tacolisp code when necessary
...
- Removing `logging.basicConfig()` from `TacoServer.__init__()`
...
- Log `taco_full_version_str` in daemon and all services
...
- Substantially reduced overall size of Taco packages
...
- Make sure taco commands output help when no args are given (#11013) (Thanks @noneus!)
...
## 1.5.0 Taco blockchain 2022-7-26
...
- Update `taco wallet take_offer` to show NFT royalties that will be paid out when an offer is taken
- Added a parameter to indicate how many additional puzzle hashes `create_more_puzzle_hashes` should create

### Fixed

- Fixed [CVE-2022-36447] where in tokens previously minted on the Taco blockchain using the `CAT1` standard can be inflated in arbitrary amounts by any holder of the token. Total amount of the token can be increased as high as the malicious actor pleases. This is true for every `CAT1` on the Taco blockchain, regardless of issuance rules. This attack is auditable on-chain, so maliciously altered coins can potentially be "marked" by off-chain observers as malicious.
...
## 1.4.0 Taco blockchain 2022-6-29

### Added

- Added support for NFTs!!! :party:
- Added `taco wallet nft` command (see <https://docs.taconetwork.net/docs/13cli/did_cli>)
- Added `taco wallet did` command (see <https://docs.taconetwork.net/docs/12rpcs/nft_rpcs>)
- Added RPCs for DID (see <https://docs.taconetwork.net/docs/12rpcs/did_rpcs>)
- Added RPCs for NFT (see <https://docs.taconetwork.net/docs/12rpcs/nft_rpcs>)
...
- Added CLI options `--sort-by-height` and –sort-by-relevance` to `taco wallet get_transactions`
...
- New coin selection algorithm based on bitcoin knapsack. Previously taco selected the largest coin
...
- Updated warning about `TACO_ROOT` being set when running init
...
## 1.3.5 Taco blockchain 2022-5-11
...
## 1.3.4 Taco blockchain 2022-4-19
...
- Added `-n`/`--new-address` option to `taco wallet get_address`
...
- Fixed typo in command line argument parsing for `taco db validate`
- Improved backwards compatibility for node RPC calls `get_blockchain_state` and `get_additions_and_removals`
- Fixed issue where `--root_path` option was not honored by `taco configure` CLI command
- Fixed cases where node DB was not created initially using v2 format
- Improved error messages from `taco db upgrade`
- Capitalized display of `Rpc` -> `RPC` in `taco show -s` by (Thanks @hugepants)
...
- Minor output formatting/enhancements for `taco wallet show`
- Fixed typo and index issues in wallet database
- Used the rust clvm version instead of python in more places
- Fixed trailing bytes shown in CAT asset ID row when using `taco wallet show`
...
- You cannot install and run taco blockchain using the macOS packaged DMG on macOS Mojave (10.14).
...
## 1.3.3 Taco blockchain 2022-4-02
...
- Fixed exception when `taco keys migrate` is run without needing migration.
...
- Added checks to ensure wallet address prefixes are either `xtx` or `txtx`.
...
## 1.3.0 Taco blockchain 2022-3-07
...
- Added new RPC, get_version, to the daemon to return the version of Taco (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores Taco will not use when launching process pools. Using 0 will allow Taco to use all cores for process pools. Set the default to 0 to allow Taco to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
- Added new RPC, get_logged_in_fingerprint, to the wallet to return the currently logged in fingerprint.
- Added new CLI option, taco keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using taco keys derive search.
...
- CLI option taco show will display the currently selected network (mainnet or testnet).
- CLI option taco plots check will display the Pool Contract Address for Portable (PlotNFT) plots.
...
- Fixed issues with displaying IPv6 address in CLI with taco show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the taco-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Fixed display info in CLI taco show -c when No Info should be displayed.
...
- When you close the Taco app, regardless if you are in farmer mode or wallet, the content on the exit dialog isn’t correct.
- If you start with wallet mode and then switch to farmer mode and back to wallet mode, the full node will continue to sync in the background. To get the full node to stop syncing after switching to wallet mode, you will need to close the Taco and relaunch the Taco app.
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the Taco Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `taco init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `taco plotnft join`) now accept a fee parameter, but it is not yet operable.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other taco services.
...
- Updated the old comment on taco/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the Taco DID wallet that showed 'mojo:'' instead of 'mojo'.
...
- Added an option to sign bytes as well as UTF-8 strings, which is particularly helpful if you're writing Tacolisp puzzles that require signatures and you want to test them without necessarily writing a whole python script for signing the relevant data.
...
- Included all Tacolisp files in source distribution.
...
- All Taco repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `taco plotnft` to manage pools.
...
- We have increased the strictness of validating Tacolisp in the mempool and clvm.
...
- Correctly display private key in `taco keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `taco keys show`.
...
- Thank you to @TacoMineJP for various improvements.
...
- `taco farm summary` better handles wallet errors.
...
- Mainnet uses port 18620 and other constants and service names were changed for mainnet.
...
- This is a hard fork/breaking change from RC6/7. TXTX Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.taconetwork.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `taco farm summary` will now display TXTX or XTX as appropriate.
...
- The Farm page will now no longer get stuck at 50 TXTX farmed.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.taconetwork.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.taconetwork.net via https, for example.
...
- RC3 is a new chain to support the last major tacolisp changes. TXTX from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- The Beta 27 chain is a hard fork. All TXTX from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- The cli wallet, `taco wallet`, was incorrectly displaying TXTX balances as if they were Coloured Coins.
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much TXTX you've won so far. Harvester and Full Node connections have moved to Advanced Options.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Taco Network at backup.taconetwork.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Taco backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.taconetwork.net, your own installation, or a third party's version of it.
...
- Fixed a typo in `taco show -w` with thanks to @pyl on Keybase.
...
- Changed XTX units to TXTX units for testnet.
...
- The user interface now refers to taco coins with their correct currency code of XTX.
...
- Implemented an RPC interface with JSON serialization for streamables - currently on port 18735.
...
- More details on the release at [https://www.taconetwork.net/developer/](https://www.taconetwork.net/developer/)
