...
- Added `taco wallet nft` command (see https://docs.taco-network.net/docs/13cli/did_cli)
- Added `taco wallet did` command (see https://docs.taco-network.net/docs/12rpcs/nft_rpcs)
- Added RPCs for DID (see https://docs.taco-network.net/docs/12rpcs/did_rpcs)
- Added RPCs for NFT (see https://docs.taco-network.net/docs/12rpcs/nft_rpcs)
...
- Beta builds are built on every merge to main, and are now available from <https://taco-network.net/download/>.
...
- Switched IP address lookup to first use Taco's service ip.taco-network.net.
...
- This is a hard fork/breaking change from RC6/7. TCAC Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.taco-network.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `taco farm summary` will now display TCAC or CAC as appropriate.
...
- We have moved to a new and much higher scalability download.taco-network.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.taco-network.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.taco-network.net/latest/Setup-MacOS.dmg).
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.taco-network.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.taco-network.net via https, for example.
...
- RC3 is a new chain to support the last major tacolisp changes. TCAC from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- 'taco wallet send' now takes in TCAC or CAC as units instead of mojos.
...
- The cli wallet, `taco wallet`, was incorrectly displaying TCAC balances as if they were Coloured Coins.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [taco-network.net](https://taco-network.net/). Among the improvements this gives the Taco blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Taco Network at backup.taco-network.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Taco backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.taco-network.net, your own installation, or a third party's version of it.
...
- The user interface now refers to taco coins with their correct currency code of CAC.
...
- New AJAX based full node UI. To access go to [http://127.0.0.1:18735/index.html](http://127.0.0.1:18735/index.html) with any modern web browser on the same machine as the full node.
...
- More details on the release at [https://www.taco-network.net/developer/](https://www.taco-network.net/developer/)
