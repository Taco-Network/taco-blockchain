...
from taco.util.api_decorators import get_metadata
from taco.util.errors import Err, ProtocolError
from taco.util.ints import uint16
from taco.util.network import WebServer, is_in_network, is_localhost
...
class TacoServer:
    ...
    all_connections: Dict[bytes32, WSTacoConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        taco_ca_crt_path, taco_ca_key_path = taco_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                taco_ca_crt_path, taco_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                taco_ca_crt_path, taco_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Callable = None):
        if self.webserver is not None:
            raise RuntimeError("TacoServer already started")
    ...
    async def incoming_connection(self, request):
        ...
        try:
            connection = WSTacoConnection.create(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    def connection_closed(self, connection: WSTacoConnection, ban_time: int, closed_connection: bool = False):
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        try:
            ...
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.taconetwork.net/") as resp:
                    if resp.status == 200:
