...
from taco.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def get_taco_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "taco_ca.crt")
    key = pkg_resources.resource_string(__name__, "taco_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Taco"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Taco"),
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Taco"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Taco CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(taco_ca_crt_path, taco_ca_crt, taco_ca_key_path, taco_ca_key, overwrite=overwrite)
...
def main():
    return make_ca_cert(Path("./taco_ca.crt"), Path("./taco_ca.key"))
