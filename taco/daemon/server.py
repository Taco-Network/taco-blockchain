...
from taco.cmds.init_funcs import check_keys, taco_init, taco_full_version_str
...
from taco.plotters.plotters import get_available_plotters
...
from taco.util.beta_metrics import BetaMetricsLogger
from taco.util.taco_logging import initialize_service_logging
from taco.util.config import load_config
from taco.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
...
from taco.util.lock import Lockfile, LockfileError
from taco.util.network import WebServer
...
if getattr(sys, "frozen", False):
    name_map = {
        "taco": "taco",
        "taco_data_layer": "start_data_layer",
        "taco_data_layer_http": "start_data_layer_http",
        ...
        "taco_seeder": "start_seeder",
        "taco_crawler": "start_crawler",
    }
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Taco will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with TACO_ROOT: {os.environ['TACO_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"taco-blockchain version: {taco_full_version_str()}")
