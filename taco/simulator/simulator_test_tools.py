...
from taco.daemon.server import WebSocketServer, daemon_launch_lock_path
from taco.simulator.full_node_simulator import FullNodeSimulator
from taco.simulator.socket import find_available_listen_port
from taco.simulator.ssl_certs import (
...
from taco.simulator.start_simulator import async_main as start_simulator_main
from taco.ssl.create_ssl import create_all_ssl
from taco.types.blockchain_format.sized_bytes import bytes32
from taco.util.bech32m import encode_puzzle_hash
from taco.util.config import create_default_taco_config, load_config, save_config
from taco.util.errors import KeychainFingerprintExists
from taco.util.ints import uint32
from taco.util.keychain import Keychain
from taco.util.lock import Lockfile
...
def create_config(
    ...
    # create taco directories
    create_default_taco_config(taco_root)
    create_all_ssl(
        taco_root,
    ...
    # load config
    config = load_config(taco_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "txtx")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(taco_root, "config.yaml", config)
    return config


async def start_simulator(taco_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=taco_root)
    await service.start()
...
async def get_full_taco_simulator(
    taco_root: Path,
    ...
    """
    A taco root Path is required.
    The taco root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the taco test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(taco_root)):
        ...
        crt_path = taco_root / config["daemon_ssl"]["private_crt"]
        key_path = taco_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = taco_root / config["private_ssl_ca"]["crt"]
        ca_key_path = taco_root / config["private_ssl_ca"]["key"]

        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(taco_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.setup_process_global_state()
        await ws_server.start()

        async for simulator in start_simulator(taco_root, automated_testing):
            yield simulator, taco_root, config, mnemonic, fingerprint, keychain
