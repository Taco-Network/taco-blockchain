...
class FullNode:
    ...
    def set_server(self, server: TacoServer):
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.taconetwork.org")
        try:
    ...
    async def respond_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, '.com/superdutyf3/taco-blockchain')}, "
                f"validation time: {validation_time}, "
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
