...
DEFAULT_USER = f"user-taco-{CURRENT_KEY_VERSION}"  # e.g. user-taco-1.8
DEFAULT_SERVICE = f"taco-{DEFAULT_USER}"  # e.g. taco-user-taco-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting TACO_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("TACO_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from taco.cmds.passphrase_funcs import prompt_for_passphrase
