...
from taco.util.db_version import set_db_version
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xtx_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xtx_target_address"):
        ...
        if "xtx_target_address" not in config["farmer"]:
            print(
                f"Setting the xtx destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
...
def taco_init(
    ...
    *,
    ...
    """
    taco_root = os.environ.get("TACO_ROOT", None)
    if taco_root is not None:
        print(f"TACO_ROOT is set to {taco_root}")
