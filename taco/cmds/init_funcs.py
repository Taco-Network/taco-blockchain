...
def check_keys(new_root: Path) -> None:
    ...
    all_targets = []
    stop_searching_for_farmer = ".com/superdutyf3/taco-blockchain_target_address" not in config["farmer"]
    stop_searching_for_pool = ".com/superdutyf3/taco-blockchain_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get(".com/superdutyf3/taco-blockchain_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get(".com/superdutyf3/taco-blockchain_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if ".com/superdutyf3/taco-blockchain_target_address" not in config["farmer"]:
        print(f"Setting the .com/superdutyf3/taco-blockchain destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"][".com/superdutyf3/taco-blockchain_target_address"] = all_targets[0]
    elif config["farmer"][".com/superdutyf3/taco-blockchain_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['.com/superdutyf3/taco-blockchain_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if ".com/superdutyf3/taco-blockchain_target_address" not in config["pool"]:
        print(f"Setting the .com/superdutyf3/taco-blockchain destination address for coinbase reward to {all_targets[0]}")
        config["pool"][".com/superdutyf3/taco-blockchain_target_address"] = all_targets[0]
    elif config["pool"][".com/superdutyf3/taco-blockchain_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['.com/superdutyf3/taco-blockchain_target_address']} with {all_targets[0]}"
        )
