...
from taco.daemon.client import acquire_connection_to_daemon
from taco.util.config import load_config
from taco.util.errors import KeychainMaxUnlockAttempts
from taco.util.keychain import Keychain, supports_os_passphrase_storage
from taco.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE, KeyringWrapper
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Taco processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("TACO_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'taco passphrase set' or 'taco passphrase remove' to update or remove your passphrase")
        sys.exit(1)
