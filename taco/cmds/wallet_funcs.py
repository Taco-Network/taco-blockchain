...
from taco.cmds.cmds_util import transaction_submitted_msg, transaction_status_msg
from taco.cmds.show import print_connections
...
from taco.wallet.nft_wallet.nft_info import NFT_HRP, NFTInfo
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if typ == WalletType.STANDARD_WALLET:
        ...
        final_min_coin_amount = uint128(int(min_coin_amount * units["taco"]))
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if nft_coin_id is not None and nft_royalty_percentage > 0:
        ...
        nft_royalty_divisor = units["taco"] if nft_royalty_asset_id == "taco" else units["cat"]
