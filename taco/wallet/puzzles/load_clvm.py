...
recompile_requested = (os.environ.get("TACO_DEV_COMPILE_CLVM_ON_IMPORT", "") != "") or ("pytest" in sys.modules)
...
def load_serialized_clvm(
    ...
    # Set the TACO_DEV_COMPILE_CLVM_ON_IMPORT environment variable to anything except
    # "" or "0" to trigger automatic recompilation of the Tacolisp on load.
    if recompile:
        try:
            if pkg_resources.resource_exists(package_or_requirement, clvm_filename):
                ...
                if include_standard_libraries:
                    ...
                    taco_puzzles_path = pathlib.Path(pkg_resources.resource_filename(__name__, "__init__.py")).parent
                    search_paths.append(taco_puzzles_path)
