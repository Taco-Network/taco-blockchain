...
from taco.types.weight_proof import SubEpochData, WeightProof
from taco.util.byte_types import hexstr_to_bytes
from taco.util.chunks import chunks
from taco.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from taco.util.default_root import STANDALONE_ROOT_PATH
from taco.util.ints import uint32, uint64
from taco.util.keychain import Keychain, KeyringIsLocked
...
class WalletNode:
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.taco-network.net"]),
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSTacoConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def fetch_puzzle_solution(self, peer: WSTacoConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSTacoConnection] = None
    ) -> List[CoinState]:
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
            raise ValueError("Not connected to the full node")
    ...
    async def fetch_children(
        self, peer: WSTacoConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
