...
from taco.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from taco.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            taco_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        taco_tx = dataclasses.replace(taco_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(taco_tx)
    ...
    async def create_tandem_xtx_tx(
        ...
        if fee > amount_to_claim:
            taco_coins = await self.standard_wallet.select_coins(
                fee,
                min_coin_amount=min_coin_amount,
                max_coin_amount=max_coin_amount,
                excluded_coin_amounts=exclude_coin_amounts,
            )
            origin_id = list(taco_coins)[0].name()
            ...
            message = None
            for spend in taco_tx.spend_bundle.coin_spends:
            ...
            selected_amount = sum([c.amount for c in taco_coins])
            taco_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XTX as part of this
        regular_taco_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_taco_to_claim = payment_amount

        need_taco_transaction = (fee > 0 or regular_taco_to_claim > 0) and (fee - regular_taco_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_taco_transaction:
                    if fee > regular_taco_to_claim:
                        taco_tx, _ = await self.create_tandem_xtx_tx(
                            fee,
                            uint64(regular_taco_to_claim),
                    ...
                    elif regular_taco_to_claim > fee:
                        taco_tx, _ = await self.create_tandem_xtx_tx(
                            fee,
                            uint64(regular_taco_to_claim),
                            min_coin_amount=min_coin_amount,
        ...
        taco_spend_bundle = SpendBundle([], G2Element())
        if taco_tx is not None and taco_tx.spend_bundle is not None:
            taco_spend_bundle = taco_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if taco_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=taco_tx.confirmed_at_height,
                    created_at_time=taco_tx.created_at_time,
                    to_puzzle_hash=taco_tx.to_puzzle_hash,
                    amount=taco_tx.amount,
                    fee_amount=taco_tx.fee_amount,
                    confirmed=taco_tx.confirmed,
                    sent=taco_tx.sent,
                    spend_bundle=None,
                    additions=taco_tx.additions,
                    removals=taco_tx.removals,
                    wallet_id=taco_tx.wallet_id,
                    sent_to=taco_tx.sent_to,
                    trade_id=taco_tx.trade_id,
                    type=taco_tx.type,
                    name=taco_tx.name,
                    memos=[],
                )
