...
class CATWallet:
    ...
    async def create_tandem_xtx_tx(
        ...
        if fee > amount_to_claim:
            taco_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            ...
            if first:
                ...
                if need_taco_transaction:
                    if fee > regular_taco_to_claim:
                        ...
                        taco_tx, _ = await self.create_tandem_xtx_tx(
                            fee, uint64(regular_taco_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
