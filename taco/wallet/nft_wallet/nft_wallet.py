...
from taco.wallet.nft_wallet import nft_puzzles
from taco.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from taco.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from taco.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from taco.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from taco.wallet.payment import Payment
from taco.wallet.puzzle_drivers import PuzzleInfo, Solver
from taco.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from taco.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
from taco.wallet.transaction_record import TransactionRecord
from taco.wallet.uncurried_puzzle import uncurry_puzzle
from taco.wallet.util.compute_memos import compute_memos
from taco.wallet.util.debug_spend_bundle import disassemble
from taco.wallet.util.transaction_type import TransactionType
from taco.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
from taco.wallet.wallet_nft_store import WalletNftStore
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSTacoConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSTacoConnection) -> None:
    ...
    async def sign_message(self, message: str, nft: NFTCoinInfo) -> Tuple[G1Element, G2Element]:
        ...
        if uncurried_nft is not None:
            ...
            synthetic_secret_key = calculate_synthetic_secret_key(private, DEFAULT_HIDDEN_PUZZLE_HASH)
            synthetic_pk = synthetic_secret_key.get_g1()
            puzzle: Program = Program.to(("Taco Signed Message", message))
    ...
    async def create_tandem_xtx_tx(
        self, fee: uint64, announcement_to_assert: Optional[Announcement] = None
    ) -> TransactionRecord:
        taco_coins = await self.standard_wallet.select_coins(fee)
        taco_tx = await self.standard_wallet.generate_signed_transaction(
            uint64(0),
        ...
        assert taco_tx.spend_bundle is not None
        return taco_tx
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, taco_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if taco_tx is not None and taco_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, taco_tx.spend_bundle])
            taco_tx = dataclasses.replace(taco_tx, spend_bundle=None)
        ...
        if taco_tx is not None:
            tx_list.append(taco_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            taco_tx = await self.create_tandem_xtx_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, taco_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'XTX'} to offer")
    ...
    async def mint_from_did(
        ...
        xtx_coins: Optional[Set[Coin]] = None,
        xtx_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an xtx spend that funds the transactions and pays fees.
        ...
        :param xtx_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param xtx_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the xtx spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an xtx coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if xtx_coins is None:
            xtx_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(xtx_coins) > 0
        ...
        # Create the xtx spend to fund the minting.
        spend_value = sum([coin.amount for coin in xtx_coins])
        change: uint64 = uint64(spend_value - total_amount)
        xtx_spends = []
        if xtx_change_ph is None:
            xtx_change_ph = await self.standard_wallet.get_new_puzzlehash()
        xtx_primaries = [
            AmountWithPuzzlehash({"puzzlehash": xtx_change_ph, "amount": change, "memos": [xtx_change_ph]})
        ]

        first = True
        for xtx_coin in xtx_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(xtx_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in xtx_coins]
                message_list.append(
                    Coin(xtx_coin.name(), xtx_primaries[0]["puzzlehash"], xtx_primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(xtx_coins) > 1:
                    xtx_announcement: Optional[Set[bytes]] = {message}
                else:
                    xtx_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=xtx_primaries,
                    fee=fee,
                    coin_announcements=xtx_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(xtx_coin.name(), message).name()
            ...
            xtx_spends.append(CoinSpend(xtx_coin, puzzle, solution))
        xtx_spend = await self.standard_wallet.sign_transaction(xtx_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, xtx_spend, *eve_spends])
        return total_spend

    async def mint_from_xtx(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        """
        Minting NFTs from a single XTX spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = xtx_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from XTX to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the XTX spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the XTX spend
            ...
            # Annnouncements for eve spend. These are asserted by the xtx spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for xtx_coin in xtx_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=xtx_primaries + primaries,
                    fee=fee,
                    coin_announcements=xtx_announcement if len(xtx_coins) > 1 else None,
...
if TYPE_CHECKING:
    from taco.wallet.wallet_protocol import WalletProtocol
