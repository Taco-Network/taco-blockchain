...
from taco.wallet.notification_store import Notification, NotificationStore
from taco.wallet.transaction_record import TransactionRecord
from taco.wallet.util.compute_memos import compute_memos_for_spend
from taco.wallet.util.notifications import construct_notification
...
class NotificationManager:
    ...
    async def send_new_notification(
        ...
        taco_tx = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
            amount,
            notification_hash,
            fee,
            coins=coins,
        ...
        full_tx: TransactionRecord = dataclasses.replace(
            taco_tx, spend_bundle=SpendBundle.aggregate([taco_tx.spend_bundle, extra_spend_bundle])
        )
        return full_tx
