...
class Wallet:
    ...
    async def create_spend_bundle_relative_taco(self, taco_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        ...
        if taco_amount < 0:
            utxos = await self.select_coins(uint64(abs(taco_amount)), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(taco_amount), "memos": []}
                ]
    ...
    # WSTacoConnection is only imported for type checking
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSTacoConnection
