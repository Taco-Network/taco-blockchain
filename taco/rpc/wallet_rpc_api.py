...
from taco.data_layer.data_layer_wallet import DataLayerWallet
from taco.pools.pool_wallet import PoolWallet
from taco.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from taco.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
...
from taco.types.blockchain_format.coin import Coin, coin_as_list
...
from taco.util.errors import KeychainIsLocked
from taco.util.ints import uint8, uint32, uint64, uint16
...
from taco.wallet.cat_wallet.cat_constants import DEFAULT_CATS
...
from taco.wallet.did_wallet.did_wallet_puzzles import match_did_puzzle, program_to_metadata
from taco.wallet.nft_wallet import nft_puzzles
from taco.wallet.nft_wallet.nft_info import NFTInfo, NFTCoinInfo
from taco.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from taco.wallet.nft_wallet.nft_wallet import NFTWallet
from taco.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from taco.wallet.notification_store import Notification
from taco.wallet.outer_puzzles import AssetType
from taco.wallet.puzzle_drivers import PuzzleInfo, Solver
from taco.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
...
from taco.wallet.util.address_type import AddressType, is_valid_address
...
class WalletRpcApi:
    ...
    async def get_latest_coin_spend(
        self, peer: Optional[WSTacoConnection], coin_id: bytes32, latest: bool = True
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from taco.util.bech32m import bech32_decode, convertbits
        from taco.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def check_offer_validity(self, request) -> EndpointResult:
        offer_hex: str = request["offer"]
        offer = Offer.from_bech32(offer_hex)
        peer: Optional[WSTacoConnection] = self.service.get_full_node_peer()
        if peer is None:
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xtx")
        ...
        key = None
        if asset_id is not None and asset_id != "xtx":
    ...
    async def did_get_info(self, request) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.XTX.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        xtx_coin_list = request.get("xtx_coins", None)
        xtx_coins = None
        if xtx_coin_list:
            xtx_coins = set([Coin.from_json_dict(xtx_coin) for xtx_coin in xtx_coin_list])
        xtx_change_target = request.get("xtx_change_target", None)
        if xtx_change_target is not None:
            if xtx_change_target[:2] == "xtx":
                xtx_change_ph = decode_puzzle_hash(xtx_change_target)
            else:
                xtx_change_ph = bytes32(hexstr_to_bytes(xtx_change_target))
        else:
            xtx_change_ph = None
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_xtx(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
