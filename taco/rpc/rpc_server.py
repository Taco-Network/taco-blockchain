...
from taco.rpc.util import wrap_http_handler
from taco.server.outbound_message import NodeType
from taco.server.server import TacoServer, ssl_context_for_client, ssl_context_for_server
...
from taco.util.config import str2bool
from taco.util.ints import uint16
from taco.util.json_util import dict_to_json_str
from taco.util.network import WebServer
from taco.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> TacoServer:
        ...
        # a property so as to be read only which allows TacoServer to satisfy
        # Optional[TacoServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        taco.rpc.rpc_server.default_get_connections()
        """
        ...

    async def on_connect(self, peer: WSTacoConnection) -> None:
...
def default_get_connections(server: TacoServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
