...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in TACO_ROOT.
    taco_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within TACO_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("taco plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, taco_plotter_options, "tacopos", "Create a plot with the default taco plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'taco plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    ...
    if args.plotter is None:
        ...
        plot_bladebit(args, taco_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(taco_root_path)
...
def show_plotters_version(root_path: Path):
    ...
    if "tacopos" in info and "version" in info["tacopos"]:
        print(f"tacopos: {info['tacopos']['version']}")
