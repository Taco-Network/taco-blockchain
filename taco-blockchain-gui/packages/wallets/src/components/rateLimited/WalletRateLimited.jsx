import { AlertDialog, Card, Flex, tacoToMojo, mojoToTacoLocaleString } from '@taco/core';
...
function IncompleteCard(props) {
  ...
  function submit() {
    ...
    dispatch(rl_set_user_info_action(id, interval_value, tacoper_value, origin_parsed, admin_pubkey_input));
  }
      ...
      <Card title={<Trans>Rate Limited Info</Trans>}>
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>Spending Limit (taco per interval): {mojoToTacoLocaleString(limit)}</Trans>
              </Typography>
            </Box>
          </Box>
        </Grid>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToTacoLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function SendCard(props) {
  ...
  function send() {
    ...
    const amount = tacoToMojo(amount_input.value);
    const fee = tacoToMojo(fee_input.value);
