...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToTacoLocaleString, mojoToCATLocaleString } from '@taco/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@taco/api-react';
import styled from 'styled-components';
import type { Row } from '@taco/core';
import { WalletType, TransactionType } from '@taco/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToTacoLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToTacoLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
