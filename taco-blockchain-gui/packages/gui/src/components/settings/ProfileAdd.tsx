import { useCreateNewWalletMutation, useGetWalletBalanceQuery } from '@taco/api-react';
import { ButtonLoading, tacoToMojo, EstimatedFee, Flex, Form, mojoToTacoLocaleString } from '@taco/core';
...
export default function ProfileAdd() {
  ...
  function handleClick() {
    openExternal('https://faucet.taconetwork.net/');
  }
  ...
  async function handleSubmit(data: CreateProfileData) {
    ...
    const walletId = await createProfile({
      ...
      options: { did_type: 'new', backup_dids: [], num_of_backup_ids_needed: '0', amount: 1, fee: tacoToMojo(fee) },
    }).unwrap();

    navigate(`/dashboard/settings/profiles/${walletId}`);
  }

  const standardBalance = mojoToTacoLocaleString(balance?.confirmedWalletBalance);
        ...
        <StyledCard>
          ...
          <Flex flexDirection="column" gap={2.5} paddingBottom={3}>
            <Typography variant="caption">
              <Trans>Balance: {standardBalance} XTX</Trans>
            </Typography>
          </Flex>
          <Flex flexDirection="column" gap={2.5} paddingBottom={1}>
            <EstimatedFee
          ...
          <Flex flexDirection="column" gap={2.5} paddingBottom={3}>
            <Typography variant="caption">
              <Trans>Recommended: 0.000005 XTX</Trans>
            </Typography>
          </Flex>
