...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToTaco, mojoToCAT } from '@taco/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xtx', 'txtx'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToTaco(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xtx', 'txtx'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToTaco(takerEntries[0][1])
