...
import { OfferSummaryRecord, OfferTradeRecord } from '@taco/api';
import { useCheckOfferValidityMutation, useGetNFTInfoQuery, useGetNFTWallets } from '@taco/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['xtx', 'txtx'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.TACO: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const xtxMakerFee = mojoToTaco(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(xtxMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.TACO
        ? tacoToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
