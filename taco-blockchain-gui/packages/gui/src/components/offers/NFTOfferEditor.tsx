...
/*        An NFT offer consists of a single NFT being offered for XTX         */
/* ========================================================================== */
...
function NFTOfferConditionalsPanel(props: NFTOfferConditionalsPanelProps) {
  ...
  const amount = methods.watch('xtxAmount');
  ...
  const offerElem =
    tab === NFTOfferExchangeType.NFTForXTX ? nftElem : amountElem;
  const takerElem =
    tab === NFTOfferExchangeType.NFTForXTX ? amountElem : nftElem;
  ...
  function handleAmountChange(amount: string) {
    methods.setValue('xtxAmount', amount);
  }
  ...
  return (
    ...
    >
      ...
      >
        <Tab
          value={NFTOfferExchangeType.NFTForXTX}
          label={<Trans>NFT for XTX</Trans>}
          disabled={disabled}
        />
        <Tab
          value={NFTOfferExchangeType.XTXForNFT}
          label={<Trans>XTX for NFT</Trans>}
          disabled={disabled}
        />
      </Tabs>
      <Grid container>
        <Flex flexDirection="column" flexGrow={1} gap={3}>
          ...
          {tab === NFTOfferExchangeType.XTXForNFT && (
            <Flex flexDirection="column" gap={2}>
              ...
              <>
                <Flex flexDirection="column" gap={0.5}>
                  <Typography variant="body1" color="textSecondary">
                    {tab === NFTOfferExchangeType.NFTForXTX ? (
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*             Currently only supports a single NFT <--> XTX offer            */
/* ========================================================================== */
...
function buildOfferRequest(
  ...
  const baseMojoAmount: BigNumber = tacoToMojo(xtxAmount);
  const mojoAmount =
    exchangeType === NFTOfferExchangeType.NFTForXTX
      ? baseMojoAmount
      : baseMojoAmount.negated();
  const feeMojoAmount = tacoToMojo(fee);
  const nftAmount = exchangeType === NFTOfferExchangeType.NFTForXTX ? -1 : 1;
  const xtxWalletId = 1;
  ...
  return [
    {
      ...
      [xtxWalletId]: mojoAmount,
    },
    exchangeType === NFTOfferExchangeType.XTXForNFT ? driverDict : undefined,
    // driverDict,
    feeMojoAmount,
  ];
}
...
export default function NFTOfferEditor(props: NFTOfferEditorProps) {
  ...
  function validateFormData(
    ...
    const { exchangeType, nftId, xtxAmount, fee } = unvalidatedFormData;
  ...
  async function handleSubmit(unvalidatedFormData: NFTOfferEditorFormData) {
    ...
    const { exchangeType, launcherId, xtxAmount, fee } = formData;

    if (exchangeType === NFTOfferExchangeType.NFTForXTX) {
      const haveNFT =
