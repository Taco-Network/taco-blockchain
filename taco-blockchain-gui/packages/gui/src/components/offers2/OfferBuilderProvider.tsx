import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@taco/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@taco/api';
import { useCalculateRoyaltiesForNFTsQuery, useGetNFTsByNFTIDsQuery, useGetWalletsQuery } from '@taco/api-react';
import { catToMojo, tacoToMojo } from '@taco/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedXTX = useWatch({
    name: 'requested.xtx',
  });

  const offeredXTX = useWatch({
    name: 'offered.xtx',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedXTX ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('xtx', tacoToMojo(amount))),
    ...(requestedTokens ?? [])
