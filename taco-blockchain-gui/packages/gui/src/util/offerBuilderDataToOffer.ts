...
import { tacoToMojo, catToMojo } from '@taco/core';
...
export default async function offerBuilderDataToOffer(
  ...
  const {
    offered: { xtx: offeredXch = [], tokens: offeredTokens = [], nfts: offeredNfts = [], fee: [firstFee] = [] },
    requested: { xtx: requestedXch = [], tokens: requestedTokens = [], nfts: requestedNfts = [] },
  } = data;

  const usedNFTs: string[] = [];

  const feeInMojos = firstFee ? tacoToMojo(firstFee.amount) : new BigNumber(0);
  ...
  await Promise.all(
    offeredXch.map(async (xtx) => {
      const { amount } = xtx;
      if (!amount || amount === '0') {
        throw new Error(t`Please enter an XTX amount`);
      }
  ...
  requestedXch.forEach((xtx) => {
    ...
    walletIdsAndAmounts[wallet.id] = tacoToMojo(amount);
  });
