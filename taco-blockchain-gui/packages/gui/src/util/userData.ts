...
export function getUserDataDir(): string {
  const tacoRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(tacoRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const tacoRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${tacoRootUserDataPath}`);
  app.setPath('userData', tacoRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(tacoRootUserDataPath, 'Local Storage', 'leveldb');
