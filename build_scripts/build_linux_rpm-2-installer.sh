...
# Builds CLI only rpm
CLI_RPM_BASE="taco-blockchain-cli-$TACO_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/taco"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/taco/"
ln -s ../../opt/taco/taco "dist/$CLI_RPM_BASE/usr/bin/taco"
...
fpm -s dir -t rpm \
  ...
  --name taco-blockchain-cli \
  --license Apache-2.0 \
  --version "$TACO_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Taco is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
...
cd ../taco-blockchain-gui/packages/gui || exit 1
...
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="Taco Blockchain" \
  --config.rpm.packageName="taco-blockchain"
...
GUI_RPM_NAME="taco-blockchain-${TACO_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${TACO_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
