...
from taco.rpc.farmer_rpc_api import (
    FilterItem,
...
from taco.simulator.block_tools import get_plot_dir
from taco.simulator.time_out_assert import time_out_assert, time_out_assert_custom_interval
...
from taco.util.misc import get_list_or_len
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "xtx"), encode_puzzle_hash(new_ph_2, "xtx"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "xtx"), encode_puzzle_hash(observer_pool, "xtx")
    )
    ...
    assert config["farmer"]["xtx_target_address"] == encode_puzzle_hash(observer_farmer, "xtx")
    assert config["pool"]["xtx_target_address"] == encode_puzzle_hash(observer_pool, "xtx")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "xtx")
