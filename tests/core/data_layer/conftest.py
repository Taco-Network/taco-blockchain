...
from taco.data_layer.data_layer_util import NodeType, Status
from taco.data_layer.data_store import DataStore
from taco.types.blockchain_format.tree_hash import bytes32
...
@pytest.fixture(name="taco_daemon", scope="function")
def taco_daemon_fixture(taco_root: TacoRoot) -> Iterator[None]:
    with closing_taco_root_popen(taco_root=taco_root, args=[sys.executable, "-m", "taco.daemon.server"]):
...
@pytest.fixture(name="taco_data", scope="function")
def taco_data_fixture(taco_root: TacoRoot, taco_daemon: None, scripts_path: pathlib.Path) -> Iterator[None]:
    with closing_taco_root_popen(taco_root=taco_root, args=[os.fspath(scripts_path.joinpath("taco_data_layer"))]):
        # TODO: this is not pretty as a hard coded time
        # let it settle
        time.sleep(5)
        yield
