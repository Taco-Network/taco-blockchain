...
from taco.cmds.keys import delete_all_cmd, generate_and_print_cmd, show_cmd, sign_cmd, verify_cmd
from taco.util.config import load_config
from taco.util.file_keyring import FileKeyring
from taco.util.keychain import DEFAULT_USER, DEFAULT_SERVICE, Keychain, generate_mnemonic
...
def empty_keyring():
    with TempKeyring(user="user-taco-1.8", service="taco-user-taco-1.8") as keychain:
        yield keychain
...
class TestKeysCommands:
    ...
    def test_migration_full(self, tmp_path, setup_legacy_keyringwrapper):
        ...
        current_version_str = pkg_resources.get_distribution("taco-blockchain").version
