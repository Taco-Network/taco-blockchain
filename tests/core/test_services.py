...
from taco.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from taco.rpc.data_layer_rpc_client import DataLayerRpcClient
...
from taco.util.config import lock_and_load_config, save_config
from taco.util.ints import uint16
from taco.util.misc import sendable_termination_signals
from tests.core.data_layer.util import TacoRoot
from tests.util.misc import closing_taco_root_popen
...
@pytest.mark.asyncio
async def test_daemon_terminates(signal_number: signal.Signals, taco_root: TacoRoot) -> None:
    port = find_available_listen_port()
    with lock_and_load_config(root_path=taco_root.path, filename="config.yaml") as config:
        config["daemon_port"] = port
        save_config(root_path=taco_root.path, filename="config.yaml", config_data=config)

    with closing_taco_root_popen(taco_root=taco_root, args=[sys.executable, "-m", "taco.daemon.server"]) as process:
        client = await wait_for_daemon_connection(root_path=taco_root.path, config=config)
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        [DataLayerRpcClient.create, "taco.server.start_data_layer", "data_layer"],
        [FarmerRpcClient.create, "taco.server.start_farmer", "farmer"],
        [FullNodeRpcClient.create, "taco.server.start_full_node", "full_node"],
        [HarvesterRpcClient.create, "taco.server.start_harvester", "harvester"],
        [WalletRpcClient.create, "taco.server.start_wallet", "wallet"],
        # TODO: review and somehow test the other services too
        # [, "taco.server.start_introducer", "introducer"],
        # [, "taco.seeder.start_crawler", ""],
        # [, "taco.server.start_timelord", "timelord"],
        # [, "taco.timelord.timelord_launcher", ],
        # [, "taco.simulator.start_simulator", ],
        # [, "taco.data_layer.data_layer_server", "data_layer"],
    ],
)
@pytest.mark.asyncio
async def test_services_terminate(
    ...
    with closing_taco_root_popen(
        taco_root=taco_root,
        args=[sys.executable, "-m", "taco.daemon.server"],
    ):
        ...
        # farmer.
        daemon_client = await wait_for_daemon_connection(root_path=taco_root.path, config=config)
