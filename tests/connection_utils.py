...
from taco.protocols.shared_protocol import capabilities, protocol_version
from taco.server.outbound_message import NodeType
from taco.server.server import TacoServer, ssl_context_for_client
from taco.server.ssl_context import taco_ssl_ca_paths
...
async def disconnect_all(server: TacoServer) -> None:
...
async def disconnect_all_and_reconnect(server: TacoServer, reconnect_to: TacoServer, self_hostname: str) -> bool:
...
async def add_dummy_connection(
    server: TacoServer, self_hostname: str, dummy_port: int, type: NodeType = NodeType.FULL_NODE
) -> Tuple[asyncio.Queue, bytes32]:
    timeout = aiohttp.ClientTimeout(total=10)
    session = aiohttp.ClientSession(timeout=timeout)
    incoming_queue: asyncio.Queue = asyncio.Queue()
    config = load_config(server.root_path, "config.yaml")
    taco_ca_crt_path, taco_ca_key_path = taco_ssl_ca_paths(server.root_path, config)
    dummy_crt_path = server.root_path / "dummy.crt"
    dummy_key_path = server.root_path / "dummy.key"
    generate_ca_signed_cert(
        taco_ca_crt_path.read_bytes(), taco_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(taco_ca_crt_path, taco_ca_key_path, dummy_crt_path, dummy_key_path)
...
async def connect_and_get_peer(server_1: TacoServer, server_2: TacoServer, self_hostname: str) -> WSTacoConnection:
    """
