...
from taco.wallet.trading.offer import Offer, NotarizedPayment
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/XTX coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            taco_coins: List[Coin] = all_coins[None]
            ...
            # Create an XTX Offer for RED
            taco_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            taco_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                taco_requested_payments, taco_coins
            )
            taco_announcements: List[Announcement] = Offer.calculate_announcements(taco_requested_payments, driver_dict)
            taco_secured_bundle: SpendBundle = generate_secure_bundle(taco_coins, taco_announcements, 1000)
            taco_offer = Offer(taco_requested_payments, taco_secured_bundle, driver_dict)
            assert not taco_offer.is_valid()

            # Create a RED Offer for XTX
            ...
            new_offer = Offer.aggregate([taco_offer, red_offer, red_offer_2])
            ...
            # Create yet another offer of BLUE for XTX and RED
            ...
            assert new_offer.summary() == (
                ...
                {"xtx": 900, str_to_tail_hash("red").hex(): 350},
