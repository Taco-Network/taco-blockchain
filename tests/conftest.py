...
from taco.util.config import create_default_taco_config, lock_and_load_config
from taco.util.ints import uint16
from taco.simulator.setup_services import setup_daemon, setup_introducer, setup_timelord
from taco.util.task_timing import (
...
from taco.simulator.setup_nodes import (
...
from taco.simulator.wallet_tools import WalletTool
...
from taco.simulator.block_tools import BlockTools, test_constants, create_block_tools, create_block_tools_async
from taco.simulator.keyring import TempKeyring
from taco.simulator.setup_nodes import setup_farmer_multi_harvester
...
async def empty_blockchain(request):
    ...
    from taco.simulator.setup_nodes import test_constants
...
async def wallet_node_sim_and_wallet() -> AsyncIterator[
    Tuple[List[Union[FullNodeAPI, FullNodeSimulator]], List[Tuple[Wallet, TacoServer]], BlockTools],
]:
...
@pytest_asyncio.fixture(scope="function")
async def two_wallet_nodes_custom_spam_filtering(spam_filter_after_n_txs, xtx_spam_amount):
    async for _ in setup_simulators_and_wallets(1, 2, {}, spam_filter_after_n_txs, xtx_spam_amount):
        yield _
...
@pytest_asyncio.fixture(scope="function")
async def one_node_one_block() -> AsyncIterator[Tuple[Union[FullNodeAPI, FullNodeSimulator], TacoServer, BlockTools]]:
...
@pytest.fixture(scope="function")
def tmp_taco_root(tmp_path):
    """
    Create a temp directory and populate it with an empty taco_root directory.
    """
    path: Path = tmp_path / "taco_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_taco_root) -> Path:
    """
    Create a temp taco_root directory and populate it with a default config.yaml.
    Returns the taco_root path.
    """
    root_path: Path = tmp_taco_root
...
@pytest.fixture(name="taco_root", scope="function")
def taco_root_fixture(tmp_path: Path, scripts_path: Path) -> TacoRoot:
    root = TacoRoot(path=tmp_path.joinpath("taco_root"), scripts_path=scripts_path)
    root.run(args=["init"])
