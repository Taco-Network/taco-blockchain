...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("tacovdf").location)' 2>/dev/null)/vdf_client
TACOVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("tacovdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing tacovdf from source on Ubuntu/Debian"
		...
		echo venv/bin/python -m pip install --force --no-binary tacovdf "$TACOVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary tacovdf "$TACOVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test $RHEL_BASED; then
		echo "Installing tacovdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing tacovdf requirement boost for MacOS."
		brew install boost
		echo "Installing tacovdf from source."
