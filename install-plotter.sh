...
get_madmax_filename() {
  ...
  echo "${TACO_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
...
if [ "$PLOTTER" = "bladebit" ]; then
  ...
  mv -f "${madmax_filename}" taco_plot
  chmod 755 taco_plot
  echo "Successfully installed madmax to $(pwd)/taco_plot"
  ...
  mv -f "${madmax_filename}" taco_plot_k34
  chmod 755 taco_plot_k34
  echo "Successfully installed madmax for k34 to $(pwd)/taco_plot_k34"
else
  usage
fi
