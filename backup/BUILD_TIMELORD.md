...
The Linux and MacOS tacovdf binary wheels currently exclude an executable
required to run a [Timelord](https://github.com/Taco-Network/taco-blockchain/wiki/Timelords).
...
[install-timelord.sh](https://github.com/Taco-Network/taco-blockchain/blob/main/install-timelord.sh)
attempts to install required build dependencies for Linux and MacOS before
invoking pip to build from the source python distribution of tacovdf.

The `install-timelord.sh` install script leverages two environmental variables
that the tacovdf wheels can use to specify how to build. The service that the
